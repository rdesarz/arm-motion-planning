...
int main() {
  ...
  bool passed = true;
  passed &= require(!so101_traj_planner::detail::validate_aligator_metrics(metrics),
                    "metrics inside every planner threshold must pass");

  metrics.max_effort_limit_violation_nm = so101_traj_planner::detail::kEffortLimitToleranceNm * 2.0;
